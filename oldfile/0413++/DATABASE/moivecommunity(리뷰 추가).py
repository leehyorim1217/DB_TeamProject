...
import random
import time
from your_project.settings import TMDB_API_KEY  # API_KEY 가져오기
from movies.models import Movie  # 장고 모델 import
...
def clear_existing_data():
    ...
    cursor.execute("TRUNCATE TABLE review_comments;")
...
def save_movies(limit=10000):
    saved = 0
    for page in range(1, 501): #10000개 이상 데이터 받기 위해 반복 횟수 증가
        if saved >= limit:
            break
...
def fetch_reviews_and_save():
    cursor.execute("SELECT id, title FROM movies LIMIT 100")
    movies = cursor.fetchall()
    total_reviews = 0

    for movie_id, _ in movies:
        url = f"https://api.themoviedb.org/3/movie/{movie_id}/reviews?api_key={API_KEY}&language=ko-KR" # 영어 -> 한글로 변경 오류시 ko-KR를 en-US로 변경
        response = requests.get(url)
        ...
        for review in reviews[:3]:
            ...
            if not content.strip():
                continue
            translated_content = "🌐 " + content[:200].replace("\n", " ")
            cursor.execute("""
                INSERT INTO reviews (movie_id, author, content)
                VALUES (%s, %s, %s)
            """, (movie_id, author, translated_content))
            total_reviews += 1
    conn.commit()
    print(f"📖 총 {total_reviews}개 평론 저장 완료")
